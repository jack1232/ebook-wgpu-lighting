...
fn fs_main(in:Input) ->  @location(0) vec4f {                  
    ...
    var bp = blinnPhong(N, L, V);

    // soft-edge spot light
    var cutoff = material.cutoff * pi/180.0;
    let outerCutoff = material.outerCutoff*pi/180.0;
    let theta = dot(L, normalize(-light.lightDirection.xyz));   
    if(cutoff > outerCutoff){
        cutoff = outerCutoff;
    }
    let epsilon = cos(cutoff) - cos(outerCutoff);
    let spotIntensity = clamp((theta - cos(outerCutoff)) / epsilon, 0.0, 1.0);
    bp[0] *= spotIntensity;
    bp[1] *= spotIntensity;
