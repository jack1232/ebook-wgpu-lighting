...
const NUM_CUBES: u32 = 100;
const NUM_SPHERES: u32 = 100;
const NUM_TORI: u32 = 100;
...
impl State {
    async fn new(window: &Window, sample_count: u32) -> Self {
        ...
        let fs_shader = init
            .device
            .create_shader_module(wgpu::include_wgsl!("soft_spot_frag.wgsl"));
        ...
        let light_direction = [-8.0 as f32, -8.0, -16.0];
        let light_position = [0.0 as f32, 0.0, 1.0];
        ...
        init.queue
            .write_buffer(&light_uniform_buffer, 16, cast_slice(&light_direction));
        init.queue.write_buffer(
        ...
        let material = [0.1 as f32, 0.6, 0.4, 30.0, 1.0, 0.3, 0.3, 8.0, 18.0];
        init.queue
    ...
    fn input(&mut self, event: &WindowEvent) -> bool {
        match event {
            WindowEvent::KeyboardInput {
                ...
                VirtualKeyCode::I => {
                    self.outer_cutoff += 0.5;
                    println!("cutoff: {}", self.outer_cutoff);
                    true
                }
                VirtualKeyCode::K => {
                    self.outer_cutoff -= 0.5;
                    if self.outer_cutoff < 0.0 {
...
fn main() {
    ...
    let window = winit::window::WindowBuilder::new()
        .build(&event_loop)
        .unwrap();
    window.set_title(&*format!("{}", "soft_spot_light"));
