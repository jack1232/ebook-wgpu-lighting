...
impl State {
    async fn new(window: &Window, sample_count: u32) -> Self {
        ...
        let fs_shader = init
            .device
            .create_shader_module(wgpu::include_wgsl!("fog_frag.wgsl"));
        ...
        let fog_color: [f32; 3] = [0.5, 0.5, 0.5];
        init.queue
            .write_buffer(&light_uniform_buffer, 48, cast_slice(fog_color.as_ref()));
        ...
        let material = [0.1 as f32, 0.6, 0.4, 30.0, 15.0, 150.0];
        init.queue
    ...
    fn update(&mut self, dt: std::time::Duration) {
        ...
        let sn = 10.0 * (0.2 + dt).sin();
        let cn = 10.0 * (0.2 + dt.cos());
        self.init.queue.write_buffer(
            &self.uniform_buffers[4],
            0,
            cast_slice([2.0 * sn, 2.0 * cn, 4.0].as_ref()),
        );
...
fn main() {
    ...
    let window = winit::window::WindowBuilder::new()
        .build(&event_loop)
        .unwrap();
    window.set_title(&*format!("{}", "fog_effect"));
